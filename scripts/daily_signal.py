...
def ai_digest(items: list[Item], model: str, editorial_note: str = "") -> dict[str, Any]:
    ...
    prompt = f"""入力されたフィード候補をWeb検索で調査し、リンク先の内容を確認して中立的な日次ダイジェストを作ってください。
一次情報と公式発表を優先してください。本文を確認できない事実は断定せず、誇張やキャラクター口調を避け、簡潔で中立的に書いてください。
執筆者名、自分への言及、一人称、署名、挨拶、キャラクター口調を出さず、丁寧で簡潔な編集文体にしてください。
...
{{"title":"絵文字を含む、日付なしの短い見出し","description":"絵文字を含む80字以内の紹介","overview":"全体傾向を示す中立的な2-3文。絵文字を2〜4個含める","items":[{{"id":"入力のid","headline":"内容に合う絵文字付き日本語見出し","summary":"確認した事実をまとめる2-3文。絵文字を1〜3個含める","why_it_matters":"重要性を簡潔に示す1文。絵文字を1個含める","citations":["確認に使ったhttps URL"]}}]}}
